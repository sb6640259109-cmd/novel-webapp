...
insert into public.profiles (id, username, email, role)
select id,
  coalesce(nullif(trim(raw_user_meta_data ->> 'username'), ''), split_part(email, '@', 1) || '_' || left(id::text, 8)),
  email,
  coalesce(nullif(raw_app_meta_data ->> 'role', ''), 'READER')
from auth.users
on conflict (id) do nothing;

do $$
begin
  if to_regclass('public.users') is not null and to_regclass('public.legacy_users') is null then
    alter table public.users rename to legacy_users;
  end if;
  if to_regclass('public.legacy_users') is not null then
    alter table public.legacy_users drop column if exists password;
  end if;
end $$;
...
alter table public.author_applications drop constraint if exists author_applications_user_id_fkey;
alter table public.author_applications drop constraint if exists author_applications_reviewed_by_fkey;
alter table public.author_applications add constraint author_applications_user_id_fkey foreign key (user_id) references public.profiles(id) on delete cascade not valid;
alter table public.author_applications add constraint author_applications_reviewed_by_fkey foreign key (reviewed_by) references public.profiles(id) on delete set null not valid;
