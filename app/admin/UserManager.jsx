...
export default function UserManager() {
  ...
  const changeRole = async (userId, role) => {
    ...
    if (!window.confirm(`เปลี่ยนสิทธิ์ ${selectedUser?.email || 'ผู้ใช้นี้'} เป็น ${role} ใช่หรือไม่?`)) return;
    try {
  ...
  return <section className="mt-6 rounded-3xl border bg-white p-6 shadow-sm">
    ...
    <form onSubmit={addAuthor} className="mb-5 grid gap-3 rounded-2xl border bg-slate-50 p-4 sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_auto]"><input value={form.username} onChange={(event) => setForm((old) => ({ ...old, username: event.target.value }))} placeholder="ชื่อผู้ใช้นักเขียน" required className="rounded-lg border px-3 py-2" /><input type="email" value={form.email} onChange={(event) => setForm((old) => ({ ...old, email: event.target.value }))} placeholder="อีเมล" required className="rounded-lg border px-3 py-2" /><input type="password" minLength={8} value={form.password} onChange={(event) => setForm((old) => ({ ...old, password: event.target.value }))} placeholder="รหัสผ่านอย่างน้อย 8 ตัว" required className="rounded-lg border px-3 py-2" /><button className="rounded-lg bg-[#3F6FAF] px-4 py-2 font-semibold text-white sm:col-span-2 xl:col-span-1">เพิ่มนักเขียน</button></form>
